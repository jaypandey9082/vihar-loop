...
Product targets remain distinct from Section 6 technical and developer
...
| Accessibility semantics | Critical controls expose meaningful executable actions | Zero official labeled-target failures in tested states | 38 accessibility tests cover actions, headings, states, focus, traversal, and Privacy & data | Automatically verified |
...
| Direct-contact guardrail | Obvious phone/email/URL/social/payment values are rejected | Nine required direct-contact cases plus integration/error-copy coverage | Deterministic validator matrix passes in the 246-test suite | Automatically verified |
...
| Reproducibility | Fresh checkout runs from README | Setup, analysis, 246 tests, 38 accessibility tests, and both APK builds pass | A committed-history Section 6 clean clone passed every listed gate without local data, environment, model, or network | Automatically verified |
